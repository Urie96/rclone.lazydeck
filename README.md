...
- Cross-provider copy supports local `<->` rclone and rclone remote `<->` rclone remote through `file.lazydeck` shared clipboard and rclone `copy` / `copyto --progress` via `deck.interactive`.
